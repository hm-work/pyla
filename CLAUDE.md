...
| ファイル | 埋め込み先 | 役割 |
|---|---|---|
| `common.js` | head カスタムコード | SPA対応ユーティリティ＋全ページ共通アニメーション |
| `common.css` | head カスタムコード | 全ページ共通スタイル |
| `[page].js` | head カスタムコード | ページ固有のアニメーション・処理 |
| `[page].css` | head カスタムコード | ページ固有スタイル |
| `iframe/iframe_common.css` | Studio に直接埋め込み | 全ページ共通スタイル（Studio直書き用） |
| `iframe/iframe_[page].css` | Studio に直接埋め込み | ページ固有スタイル（Studio直書き用） |

### 埋め込み先のルール

- **`iframe_` プレフィックスあり**（`iframe/` 以下）→ Studio 上のカスタムコードブロックに直接埋め込む
- **`iframe_` プレフィックスなし**（ルート直下）→ Studio の head カスタムコードに埋め込む
- **指示がない場合は head カスタムコード（ルート直下のファイル）に書く**
